...
class GETTactileVideo():
    ...
    def __init__(self, config_csv="./config.csv", IP=None, markers=False):
        ...
        # 動作が重いときの対処法(2025-12-17)
        self.max_frames = int(self.FPS * 10)   # 例：10秒分だけ保持
        self._raw_rgb_frames = deque(maxlen=self.max_frames)
        ###
    ...
    def IP_to_URL(self, IP, port=8000):
    ...
    def start_stream(self, IP=None, plot=False, plot_diff=False, plot_depth=False, save_frames=False):
        # 追加 2025-12-17: 動画保存用
...
if __name__ == "__main__":
    GET_video = GETTactileVideo(IP="192.168.1.120", config_csv="software/config.csv")
    GET_video.start_stream(plot=True, plot_diff=True, plot_depth=False, save_frames=False)
